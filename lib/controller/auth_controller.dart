...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> updatePin(String pin) async {
    ...
    Future<void> authenticateWithBiometric(bool autoLogin, String pin) async {
      ...
      if((await _bioAuth.canCheckBiometrics || await _bioAuth.isDeviceSupported()) && authRepo.isBiometricEnabled()) {
        ...
        if (_availableBiometrics.isNotEmpty && (!autoLogin || await biometricPin() != null)) {
          try {
            ...
            if(_didAuthenticate) {
              if(autoLogin) {
                login(code: getCustomerCountryCode(), phone: getUserNumber(), password: await biometricPin());
              }else{
  ...
  Future<Response> login({String code, String phone, String password}) async {
    ...
    if (_responseModelApi.responseCode == 'auth_login_200') {
       authRepo.saveUserToken(_responseModelApi.content).then((value) async {
         Response _re = await authRepo.updateToken();
         print('update token body : ${_re.body}');
       });
